...
function DictQuickLookup:init()
    ...
    local frame_bordersize = math.ceil(Size.border.window * 1.5)
    ...
    -- No TitleBar — menus still accessible via onShowResultsMenu() if needed
    self.menu_scrolled_offsets = {}
    self.menu_opened = {}
    -- Stub dict_title so references elsewhere don't crash
    self.dict_title = {
        dimen = Geom:new{ x = 0, y = 0, w = 0, h = 0 },
        left_button = { image = { dimen = Geom:new{ x = 0, y = 0, w = 0, h = 0 } } },
        left_icon_tap_callback = function() self:onShowResultsMenu() end,
    ...
    -- Close × button (top-right, replaces TitleBar close)
    local close_button = IconButton:new{
        ...
        hold_callback = function() self:onHoldClose() end,
        show_parent = self,
    }
    local close_button_w = close_button:getSize().w

    -- Nb of results (1/2) — small text, bottom-right corner
    local lookup_word_nb_h = 0
    if self.displaynb then
        self.displaynb_text = TextWidget:new{
            text = self.displaynb,
            face = Font:getFace("cfont", math.floor(word_font_size * 0.6)),
            padding = 0,
        }
        lookup_word_nb_h = self.displaynb_text:getSize().h
    end
    -- Lookup word (bold, left) — full width, no 1/2 inline
    self.lookup_word_text = TextWidget:new{
        text = self.displayword,
        face = Font:getFace(word_font_face, word_font_size),
        bold = true,
        max_width = self.content_width - close_button_w - Size.padding.default,
        padding = 0,
        lang = self.lang_in
    }
    -- word + [×]
    ...
    -- No bottom button table — stub so all getButtonById calls return nil safely
    self.button_table = {
        layout = {},
        ...
        getFocusItem = function() return nil end,
        moveFocusTo = function() end,
    ...
    self.dict_frame = FrameContainer:new{
        ...
        VerticalGroup:new{
            ...
            -- 1/2 counter: small, bottom-right corner
            self.displaynb_text and FrameContainer:new{
                margin = 0,
                bordersize = 0,
                padding = 0,
                padding_left = inner_width - self.displaynb_text:getSize().w - content_padding_h,
                self.displaynb_text,
            ...
            callback = function()
                if self.is_wiki then
                    ...
                    button_dialog = ButtonDialog:new{
                        ...
                        width = search_btn and search_btn.dimen.w or math.floor(self.width / 2),
                        anchor = function()
                            return Geom:new{
                                x = search_btn and search_btn.dimen.x or self.dict_frame.dimen.x,
...
function DictQuickLookup:changeDictionary(index, skip_update)
    ...
    -- Remove the headword from the start of the definition.
    -- This dictionary uses: <h2><span class="hw">word</span> ...rest of h2...
    -- We strip just the hw span, leaving the pronunciation and grammar intact.
    if self.definition and self.lookupword and self.is_html then
        local escaped = self.lookupword:gsub("([%(%)%.%%%+%-%*%?%[%^%$])", "%%%1")
        -- Strip <span class="hw">word</span> (with optional space after)
        self.definition = self.definition:gsub(
            '<span class="hw">' .. escaped .. '</span>%s*', "")
        -- Also handle case-insensitive or with extra attributes
        self.definition = self.definition:gsub(
            '<span class=["\']hw["\'][^>]*>' .. escaped .. '</span>%s*', "")
    end
...
function DictQuickLookup:onShowResultsMenu()
    ...
    button_dialog = ButtonDialog:new{
        ...
        buttons = buttons,
        anchor = function()
            return self.dict_frame.dimen -- pop down
        end,
        tap_close_callback = function()
            self.menu_scrolled_offsets["main"] = button_dialog:getScrolledOffset()
            self.menu_opened[button_dialog] = nil
